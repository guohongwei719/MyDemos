//
//  GHWBaiduAdViewController.m
//  MyDemos
//
//  Created by guohongwei on 2020/1/3.
...
#define ADID_SMARTFEED @"6481011"//智能优选
...
@interface GHWBaiduAdViewController () <BaiduMobAdNativeAdDelegate,UITableViewDataSource,UITableViewDelegate>
...
@implementation GHWBaiduAdViewController
...
- (void)viewDidLoad {
    ...
    [BaiduMobAdNativeAdView dealTapGesture:YES];
    [self setupUI];
...
//    [self.tableView.mj_header endRefreshing];
    ...
    if ([self.native.adId isEqualToString:ADID_SMARTFEED]) {
        for(int i = 0; i < [nativeAds count]; i++){
            ...
            //接口未返回BaiduMobAdSmartFeedStyleType则为非智能优选广告
    ...
    for(int i = 0; i < [nativeAds count]; i++){
        ...
        // 加载和显示广告内容
        __weak GHWBaiduAdViewController *weakSelf = self;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.native.adId isEqualToString:ADID_SMARTFEED]) {
        if (self.adViewArray.count > indexPath.row) {
            BaiduMobAdSmartFeedView *adView = [self.adViewArray objectAtIndex:indexPath.row];
            return [adView viewHeight];
        }
    }
